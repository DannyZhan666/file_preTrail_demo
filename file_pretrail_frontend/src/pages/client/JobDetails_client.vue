        ...
        <el-collapse>
          ...
          <el-collapse-item title="律师相关信息" name="3">
            <el-descriptions class="margin-top" :column="1" :size="size" border>
              ...
              <el-descriptions-item label="更新时间">{{dayjs(jobInfo.updateTime).format('YYYY年MM月DD日 HH:mm:ss') }}</el-descriptions-item>
              <el-descriptions-item label="律师预期完成时间">{{dayjs(jobInfo.dueLaw).format('YYYY年MM月DD日 HH:mm:ss') }}</el-descriptions-item>
            </el-descriptions>
          </el-collapse-item>
        </el-collapse>
        <!-- 添加确认和取消按钮 -->
...
const submitForm = async () => {
  ...
  try {
    const response = await axios.post('/job/acceptJob', postData, {
      headers: {
